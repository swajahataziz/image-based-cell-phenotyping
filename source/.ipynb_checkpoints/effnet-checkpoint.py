...
def train_model(args):
    ...
    train_transform = T.Compose([
                                 ...
                                 T.Resize(size=(img_size,img_size)), # Resizing the image to be 76 by 76
    ...
    #channel_weights = torch.index_select(model.conv_stem.weight, 1, indices)
    #model.conv_stem.in_channels = 4
    #model.conv_stem.weight = torch.nn.Parameter(torch.cat([model.conv_stem.weight, channel_weights], axis=1))
    ...
    trainer.fit(model,trainloader,testloader,epochs = args.epochs)
